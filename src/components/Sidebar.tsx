import { ChevronDown, ChevronRight, Folder, FileJson, FileCode } from "lucide-react";
import { useState } from "react";

const files = {
  src: ["intro.md", "about.rs", "projects.ts", "skills.cpp", "game.tsx"],
  config: ["contact.json"],
};
      ...
      <div className="flex-1 overflow-y-auto mt-2">
        ...
        {expanded.src && files.src.map(file => (
          <div key={file} onClick={() => onOpenFile(file)} className={`flex items-center gap-2 pl-9 pr-3 py-1.5 cursor-pointer transition-colors ${activeFile === file ? "bg-[#1f6feb]/20 text-white border-l-2 border-[#58a6ff]" : "text-[#8b949e] border-l-2 border-transparent hover:bg-white/5 hover:text-[#c9d1d9]"}`}>
            <FileCode size={14} className={activeFile === file ? "text-[#58a6ff]" : "opacity-60"} /> <span>{file}</span>
          </div>
        ))}
        {/* CONFIG Directory */}
        ...
        {expanded.config && files.config.map(file => (
          <div key={file} onClick={() => onOpenFile(file)} className={`flex items-center gap-2 pl-9 pr-3 py-1.5 cursor-pointer transition-colors ${activeFile === file ? "bg-[#1f6feb]/20 text-white border-l-2 border-[#58a6ff]" : "text-[#8b949e] border-l-2 border-transparent hover:bg-white/5 hover:text-[#c9d1d9]"}`}>
            <FileJson size={14} className={activeFile === file ? "text-[#f0e130]" : "text-[#f0e130]/60"} /> <span>{file}</span>
          </div>
        ))}
      </div>
    </div>
